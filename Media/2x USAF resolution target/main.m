...
n1 = 3000; n2 = 3000;  % input image size

lambda = 0.5195;  % wavelength of illumination, assume monochromatic  HUB75 2mm pitch  R: 628.6nm  G: 519.5   B: 465.1
NA = 0.085;   % numerical aperture of the objective
mag = 2.08;  % 2.95 magnification of the system,  need to calibrate with calibration slides.    2x .08/160: 2.08     4x/inf 135mm tube lens:  2.95
dpix_c = 3.76;  %  3.76 after debayer filter Lumix G85
Np = 400;  % # of pixels at the output image patch. Each patch will assign a single k-vector, the image patch size cannot be too large to keep the single-k assumption holds

ds_led = 2e3;   %Spacing between neighboring LEDs 2mm
decimation_led = 2;  %  Use every-other LED, so it's effectively 4mm spacing
z_led = 67.5e3;  % distance from the center LED to the object
dia_led = 30;   % 30 diameter of # of LEDs used in the experiment

Ibk_thresh = 100;

lit_cenv = 40;   % set up LED coordinates
lit_cenh = 31;   %31
vled = [0:63]-lit_cenv;
hled = [0:63]-lit_cenh;
...
if(loadimages == 1)
  ...
  for m = 1:Nimg
      ...
      %tempI = double(imread(fn));
      %Iall(:,:,m) = double(imread(fn))(:,:,1);  %Read red channel from 48-bit color TIFF.
       %Iall(:,:,m) = transpose(downsample(transpose(downsample(tempI(:,:,1),2,1)),2,1));

      bk1 = mean2(double(Iall(1:100,1:100,m)))  % Specify background region coordinates
      bk2 = mean2(double(Iall(1:100,2700:2999,m)))
...
Imea = double(Iall(n1/2 - Np/2 : n1/2 + Np/2 - 1 ,n2/2 - Np/2 : n2/2 + Np/2 - 1,:)); %Use only a portion of the input images
